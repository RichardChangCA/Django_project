...
from .models import UserInfo, TeacherInfo, AdminInfo
...
def teacher_check_cookie(request):
    d = request.COOKIES.keys()
    if "qwer" in d and "asdf" in d:
        email = request.COOKIES['qwer']
        password = request.COOKIES['asdf']
        select_user = TeacherInfo.objects.filter(email=email).filter(password=password)
        if len(select_user) == 0:
            return (False, -1)
        else:
            return (True, select_user[0])
...
def admin_check_cookie(request):
    d = request.COOKIES.keys()
    if "qwer" in d and "asdf" in d:
        email = request.COOKIES['qwer']
        password = request.COOKIES['asdf']
        select_user = AdminInfo.objects.filter(email=email).filter(password=password)
        if len(select_user) == 0:
            return (False, -1)
        else:
            return (True, select_user[0])
